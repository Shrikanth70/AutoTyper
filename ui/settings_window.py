...
class SettingsWindow:
    ...
    def setup_ui(self):
        ...
        self.human_delay_var = tk.BooleanVar(
            value=self.config["typing"]["human_like_delay"]
        )
        ttk.Checkbutton(
            typing_tab,
            text="Human-like Delay (Random Variance)",
        ...
        ttk.Checkbutton(
            typing_tab,
            text="Use Paste Mode (Instantly paste text)",
        ...
        ttk.Label(hotkey_tab, text="Copy Selection:").pack(**padding)
        self.hk_copy = tk.StringVar(value=self.config["hotkeys"].get("copy_mode", "f8"))
        ttk.Entry(hotkey_tab, textvariable=self.hk_copy).pack(**padding)

        ttk.Label(hotkey_tab, text="Paste and Type Clipboard:").pack(**padding)
        ...
        ttk.Button(btn_frame, text="Save Settings", command=self.save).pack(
            side="right", padx=5
        )
    ...
    def save(self):
        try:
            new_config = {
                "hotkeys": {
                    "copy_mode": self.hk_copy.get().strip(),
...
if __name__ == "__main__":
    ...
    default_cfg = {
        "hotkeys": {
            "copy_mode": "f8",
            "clipboard_mode": "ctrl+alt+v",
            "voice_mode": "ctrl+alt+s",
            "emergency_stop": "f9",
        },
        "typing": {
            "speed": 0.05,
